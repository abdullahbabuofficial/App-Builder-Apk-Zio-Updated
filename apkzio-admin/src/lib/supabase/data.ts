...
import type { CreateAppInput, UpdateAppInput } from "@/lib/api";
...
export function mapCampaignRow(row: NotifRow): Campaign {
  return {
    ...
    active_within_minutes: row.active_window_min ?? undefined,
    country_codes: row.target_type === "country" ? row.target_countries ?? [] : undefined,
    device_ids: row.target_type === "device_list" ? row.target_device_ids ?? [] : undefined,
...
async function supabaseCurrentOwnerId(client: SupabaseClient): Promise<string> {
  const { data, error } = await client
    .from("app_owners")
    .select("owner_id")
    .single();
  if (error) throw new Error(error.message);
  const ownerId = (data as OwnerRow | null)?.owner_id;
  if (!ownerId) throw new Error("No app owner row found for this Supabase user.");
  return ownerId;
}

async function supabaseFetchAppById(client: SupabaseClient, appId: string): Promise<AndroidApp> {
  const { data, error } = await client
    .from("v_my_apps")
    .select("*")
    .eq("app_id", appId)
    .single();
  if (error) throw new Error(error.message);
  return mapAndroidApp(data as VMyAppRow, { pushCount: 0 });
}

export async function supabaseInsertApp(
  client: SupabaseClient,
  input: CreateAppInput,
): Promise<AndroidApp> {
  const ownerId = await supabaseCurrentOwnerId(client);
  const metadata = input.fcm_project_id?.trim()
    ? { fcm_project_id: input.fcm_project_id.trim() }
    : {};
  const { data, error } = await client
    .from("android_apps")
    .insert({
    ...
    .single();
  if (error) throw new Error(error.message);
  return supabaseFetchAppById(client, (data as { app_id: string }).app_id);
}

export async function supabaseUpdateApp(
  client: SupabaseClient,
  appId: string,
  patch: UpdateAppInput,
): Promise<AndroidApp> {
  const payload: Record<string, unknown> = {};
  if (patch.name !== undefined) payload.app_name = patch.name.trim();
  if (patch.status !== undefined) payload.status = patch.status === "paused" ? "archived" : patch.status;
  if (patch.fcm_project_id !== undefined) {
    const fcm = patch.fcm_project_id?.trim() || null;
    payload.fcm_project_id = fcm;
    payload.metadata = fcm ? { fcm_project_id: fcm } : {};
  }
  ...
  const { error } = await client
    ...
    .eq("app_id", appId);
  if (error) throw new Error(error.message);
  return supabaseFetchAppById(client, appId);
}

export async function supabaseDeleteApp(client: SupabaseClient, appId: string): Promise<void> {
  const { error } = await client
    .from("android_apps")
    .delete()
    .eq("app_id", appId);
