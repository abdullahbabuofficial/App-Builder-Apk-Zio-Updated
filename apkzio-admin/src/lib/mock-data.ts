...
export type WebViewBuildConfig = {
  ...
  package_name: string;
  start_url: string;
  primary_color: string;        // e.g. "#CDFF3F"
  background_color: string;     // hex
  splash_color: string;         // hex
...
export type ApkBuild = {
  ...
  triggered_by: string;
  /** Relative URL of the generated WebView source archive (e.g. /artifacts/<id>/source.zip). */
  source_zip_url?: string | null;
  /** Snapshot of the WebView config used by the builder pipeline. */
  config?: WebViewBuildConfig | null;
  /** How many real log lines the builder has emitted so far. */
  logs_count?: number;
  /**
   * Optional best-effort APK download URL (relative, e.g. /artifacts/<id>/app-debug.apk).
   * Only populated when the local-api host has JDK 17 + Gradle + ANDROID_HOME and the
   * runner's APKZIO_ENABLE_APK_BUILD env flag is set. Otherwise null and the source
   * ZIP remains the deliverable.
   */
  apk_url?: string | null;
...
// ==== Apps ====
const rng = mulberry32(hash("apkzio-admin-v1"));
...
export const BUILDS: ApkBuild[] = (() => {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    out.push({
      ...
      duration_ms: dur,
      size_bytes: size,
      output_url: status === "success" ? `https://cdn.apkzio.io/${app.id}/build-${240 - i}.apk` : null,
