...
/** Table placeholder while `/api/admin/clients` loads. */
export function ClientsDirectorySkeleton() {
  return (
    <div className="space-y-2 p-4" aria-busy aria-label="Loading clients">
      <Skeleton className="h-4 w-full max-w-md" />
      {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
        <Skeleton key={i} className="h-12 w-full rounded-lg border border-line-1/40" />
      ))}
    </div>
  );
}
