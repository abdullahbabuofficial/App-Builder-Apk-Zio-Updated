...
function exportSubscribersCsv(appName: string, rows: Subscriber[]): void {
  downloadCsv(`apkzio-subscribers-${new Date().toISOString().slice(0, 10)}.csv`, [
    ["app", "subscriber_id", "fcm_token_redacted", "valid", "device_id", "last_seen_at"],
    ...rows.map((s) => [
      appName,
      s.id,
      s.fcm_token_redacted,
      s.is_valid,
      s.device_id,
      s.last_seen_at,
    ]),
...
export function Subscribers() {
  ...
  const { dataSource, findApp } = useApkzio();
  ...
  const tokenActionsAvailable = dataSource === "rest";

  async function toggleSubscriber(s: Subscriber) {
    if (!tokenActionsAvailable) {
      toast({
        tone: "info",
        title: "Action unavailable",
        description: dataSource === "supabase"
          ? "Supabase token updates need a privileged RPC because raw tokens are protected behind a redacted view."
          : "Token actions require the REST API data source.",
      });
      return;
    }
    setUpdatingId(s.id);
    try {
      await api.updateSubscriber(s.id, { is_valid: !s.is_valid });
      toast({
        tone: "success",
        title: s.is_valid ? "Subscriber marked invalid" : "Subscriber restored",
        description: s.fcm_token_redacted,
      });
      refetch();
    } catch (e) {
      toast({
        tone: "error",
        title: "Subscriber update failed",
  ...
  const columns: Column<Subscriber>[] = [
    ...
    },
    {
      ...
      cell: (s) => (
        <Button
          variant={s.is_valid ? "danger" : "secondary"}
          size="sm"
          disabled={updatingId === s.id}
          title={tokenActionsAvailable ? undefined : "Token actions require REST mode; Supabase needs a privileged RPC."}
          onClick={() => void toggleSubscriber(s)}
        >
          {updatingId === s.id ? "Saving…" : s.is_valid ? "Invalidate" : "Restore"}
        </Button>
      ),
    },
  ];
  ...
  return (
    <>
        ...
        actions={
          <>
            <Button
              ...
              onClick={() => {
                exportSubscribersCsv(app.name, filtered);
                toast({ tone: "success", title: "Export ready", description: "Subscribers CSV downloaded." });
              }}
            >
              Export
            </Button>
      ...
      <Card>
        <CardHeader
          title={`${commas(filtered.length)} results`}
          description={tokenActionsAvailable ? "Invalidate stale tokens or restore subscribers after verification." : "Subscriber token actions require REST mode; Supabase needs a privileged RPC because tokens are protected."}
        />
