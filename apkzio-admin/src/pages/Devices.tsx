...
function exportDevicesCsv(appName: string, rows: Device[]): void {
  downloadCsv(`apkzio-devices-${new Date().toISOString().slice(0, 10)}.csv`, [
    ["app", "device_id", "manufacturer", "model", "install_hash", "country", "app_version", "os_version", "active", "first_seen_at", "last_seen_at"],
    ...rows.map((d) => [
      appName,
      d.id,
      d.manufacturer,
        ...
        actions={
          <>
            <Button
              ...
              onClick={() => {
                exportDevicesCsv(app.name, filtered);
                toast({ tone: "success", title: "Export ready", description: "Devices CSV downloaded." });
              }}
            >
              Export CSV
            </Button>
            <Button variant="secondary" leading={<Icon.Filter size={14} />} disabled title="Country and activity filters are available below.">
              More filters
            </Button>
          </>
        }
