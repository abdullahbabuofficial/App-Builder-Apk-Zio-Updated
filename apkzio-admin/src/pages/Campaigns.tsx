...
function exportCampaignsCsv(rows: ReturnType<typeof useApkzio>["campaigns"], appName: (id: string) => string): void {
  downloadCsv(`apkzio-campaigns-${new Date().toISOString().slice(0, 10)}.csv`, [
    ["id", "app", "title", "status", "target", "created_at", "scheduled_at", "sent_at", "recipients", "sent", "delivered", "opened", "clicked", "failed"],
    ...rows.map((c) => [
      c.id,
      appName(c.app_id),
      c.title,
...
export function Campaigns() {
  const { campaigns, appName } = useApkzio();
  ...
  return (
    <>
      <PageHeader
        title="Campaigns"
        description="Every push sent through ApkZio. Click a row to drill into delivery, opens, and clicks."
        actions={
          <>
            <Button
              variant="secondary"
              ...
              onClick={() => {
                exportCampaignsCsv(filtered, appName);
                toast({ tone: "success", title: "Export ready", description: "Campaigns CSV downloaded." });
              }}
            >
              Export
            </Button>
